export default {
    data(){
        return{
            fr: {
                vid_not_supported:"Votre navigateur ne supporte pas la balise vidéo.",
                nav_home: "Accueil",
                nav_ticket: "Billets",
                nav_espace_perso: "Ma Page",
                nav_exposants: "Exposants",
                title: "Super Geeky Week",
                accroche: "Bienvenue au plus grand événement geek Franc-Comtois !",
                home_titre_1: "Près de chez toi Franc-Comtois !",
                home_text_1: "Plus besoin de te déplacer jusqu'à Paris pour assister à un événement geek majeur ! Avec la Super Geeky Week tu peux maintenant profiter de tous types d'activités geek près de chez toi. Localisé dans la nouvelle salle de Belfort, la Gigantomachie, tu pourras profiter de 3 jours de folie !",
                home_titre_2: "Une myriade d'activités !",
                home_text_2: "Au programme toutes les activités dont tu peux rêver et plus encore ! Avec de prestigieux invités avec des séances d'autographes et des goodies par milliers pour être certain que tu gardes, même jusque chez toi, un souvenir inoubliable de ce week-end fantastique !",
                billet_title:"Billetterie",
                billet_text:"Pour la Super Geeky Week, nous vous proposons trois types de forfaits pour rendre votre séjour encore plus agréable. Tout d'abord, le forfait standard vous permettra de profiter pleinement de tous les stands. Ensuite, nous vous offrons le forfait prioritaire qui vous garantit un accès plus rapide à nos services, vous permettant ainsi de maximiser votre temps et votre expérience. Enfin, pour assurer une accessibilité optimale, nous avons également le forfait dédié aux personnes en situation de handicap.",
                jour:"Jour ",
                hora:" / Horaire de ",
                a:" a ",
                description:"Description : ",
                prix:"Prix : ",
                prix2:"Prix",
                prix3:"Prix :",
                stock:"Stock : ",
                prestation:"Prestations",
                quantite:"Quantité",
                soustotaux:"Sous-total",
                rentrezinfo:"Renseignez vos informations",
                prenom:"Prénom",
                nom:"Nom",
                email:"Adresse email : ",
                validerinfo:"Valider les informations",
                enteremail:"Veuillez rentrer un email valide",
                information:"Information : ",
                produit:"Produit",
                panier:"Panier",
                returnbout:"Retour boutique",
                titre:"Titre :",
                addpanier:"Ajouter au panier",
                modifier:"Modifier",
                supprimer:"Supprimer",
                valider:"Valider",
                annuler:"Annuler",
                livror:"Livre d'or",
                aucuntemoin:"Aucun témoignage disponible",
                retour:"Retour",
                desinscrire:"Désinscrire",
                horaires:"Horaires",
                inscription:"Inscription",
                zetebieninscrit:"Vous êtes bien inscrit ",
                seancede:" à la séance de ",
                filre:"Filtre",
                nopresta:"Aucun prestataire correspondant.",
                exposants:"Exposants",
                envoyer:"Envoyer",
                aucunexpos:"Aucun exposant disponible.",
                contactpresta:"Contact Prestataire",
                voirprof:"Voir le profil",
                nourriture:"Nourriture",
                activit:"Vos activités :",
                welcespace:"Bienvenue dans votre espace : ",
                servicepage:"Page de service",
                bienvenue:"Bienvenue ",
                changemdp:"Changer le mot de passe",
                dejaconnect:"Vous êtes déjà connecté",
                deconection:"Déconnexion",
                mdp:"Mot de passe",
                soumettre:"Soumettre",
                acheter:"Acheter",
                affinscrit:"Afficher inscrits",
                cachinscrit:"Cacher inscrits",
                addhoraire:"Ajouter un horaire",
                ajouter:"Ajouter",
                listinscrit:"Liste des inscrits",
                seance:"Séance",
                rechmotcle:"Recherche par mot-clé",
                mailpresta:"Mail du prestataire :",
                role:"Rôle",
                personne:"Personne",
                emplacement:"Emplacement",
                ressource:"Ressource",
                emplressources:"Emplacements des ressources",
                calendrier:"Calendrier",
                lignepanier:"Ligne du panier",
                personnetag:"Personne associée à un tag",
                event:"Événement",
                creneau:"Créneau",
                temoignage:"Témoignage",
                addtemoin:"Ajouter un témoignage",
                numcarte:"Numéro de carte",
                datecarte:"Date d'expiration",
                dontaccess:"Vous n'avez pas les droits pour accéder à cette page",
                error404:"Erreur 404",
                pgdstexist:"Cette page n'existe pas",
                mdpincorrect:"Mot de passe incorrect ! Votre mot de passe doit contenir au moins 8 caractères dont une majuscule, une minuscule, un chiffre et un caractère spécial",
                remplirchamp:"Veuillez remplir correctement les champs",
                ctctadrssuiv:'Veuillez me contacter à l\'adresse suivante :',
                yremail:'Votre adresse email :',
                yremessage:'Votre message :',
                ecrtonmessage:'Écrivez votre message ici',
                tonmail:'Votre mail',
                errordonneAPI:'Erreur: Les données de l\'API sont incorrectes ou non définies.',
                errorrecupexpo:'Erreur lors de la récupération des exposants',
                contactez:"Contactez",
                errorselecpresta:"Erreur: selectedPrestataire n'est pas défini.",
                messageenvo:"Message envoyé :",
                to:"à",
                errorenvoimsg:"Erreur lors de l'envoi du message :",
                errorsltprestavide:"Erreur: selectedPrestataire, userEmail ou messageContent est vide.",
                imgevent:"Image de l'événement",
                enterhoraire:"veuillez remplir correctement tout les champs horaires (les heures allant de 8h à 22h et les minutes de 0 à 59) et choisir un jour",
                errorname3caract:"Le nom doit contenir au moins 3 caractères et ne doit contenir aucun de chiffres",
                errorprenom3caract:"Le prenom doit contenir au moins 3 caractères et ne doit contenir aucun de chiffres",
                choiseance:"Veuillez choisir une séance",
                nodescription:"Aucune description",
                cartebnovalide:"Le numéro de carte bancaire n'est pas valide",
                datexpinovalide:"La date d'expiration n'est pas valide",
                codesecunovalide:"Le code de sécurité n'est pas valide",
                paievalide:"Paiement validé",
                imgtournoi:"Image du tournoi",
                anormalallstandtaken:"Cas anormal dans getAllStandsTaken",
                anormalinfobulle:"Cas Anormal dans GetInfoBulle",
                anormalinfopaneltake:"Cas anormal dans GetInfoPanelNoTake",
                anormalinfopanel:"Cas anormal dans GetInfoPanel",
                day:"21/22/23 Juin 2024",
                remplirtouschamps:"Veuillez remplir tous les champs",
                namerole:"Nom du rôle",
                chercher:"Chercher",
                selectligne:"Veuillez sélectionner une ligne",
                dsntdeltrole:"Vous ne pouvez pas supprimer ce rôle",
                bienvenuepage:"Bienvenue sur la page de ",
                vousprofitact:"Vous pouvez dès maintenant profiter de superbes activités",
                emplaname:"Nom de l'emplacement",
                dsntdeltrpersonne:"Vous ne pouvez pas supprimer cette personne",
                selectother:"Veuillez en sélectionner une autre",
                ressname:"Nom de la ressource",
                dsntdeltrressource:"Vous ne pouvez pas supprimer cet ressource",
                hordeb:"Horaire début",
                horfin:"Horaire fin",
                paniername:"Nom du panier",
                dsntdeltpanier:"Vous ne pouvez pas supprimer ce panier",
                dsntdeltitem:"Vous ne pouvez pas supprimer cet item",
                nomclient:"Nom du client",
                prenomclient:"Prénom du client",
                emailclient:"Email du client",
                dsntdeltqrcode:"Vous ne pouvez pas supprimer ce qrcode",
                nomtag:"Nom du tag",
                dsntdelttag:"Vous ne pouvez pas supprimer ce tag",
                nomevent:"Nom de l'événement",
                nbrplace:"Nombre de places",
                dsntdeltevent:"Vous ne pouvez pas supprimer cet événement",
                Crud:"Crud",
                Carte:"Carte",
                Statistiques:"Statistiques",
                nouvmotdepasse:"Nouveau mot de passe",
                confmotdepasse:"Confirmez le nouveau mot de passe",
                Email:"Email",
                Emailoumdpincorrect:"Email ou mot de passe incorrect",
                Contact:"Contact",
                Gratuit:"Gratuit",
                Messages:"Messages",
                Commandevide:"Commande vide",
                Utilise:"Utilisé",
                Nonutilise:"Non utilisé",
                Telecharger:"Télécharger",
                QRcodeURL:"URL du QR code : ",
                Total:"Total : ",
                Remerciementsanoscollaborateurs:"Remerciements à nos collaborateurs",
                Pourrejoindrelevenement:"Pour rejoindre l'événement",
                Item:"Item",
                QRcode:"QR code",
                Tag:"Tag",
                Stand:"Stand",
                PersonneTag:"Personne Tag",
                Description:"Description",
                Stock:"Stock",
                EmplacementRessource:"Emplacement Ressource",
                Creneau:"Créneau",
                consommer:"Consommer : ",
            },
        };
    },
};